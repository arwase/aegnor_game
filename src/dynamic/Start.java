...
public class Start {
    ...
    private class starting implements Runnable {
        ...
        public void run() {
            ...
            try {
                mapUse.put(1, World.world.getMap((short) 6824).getMapCopyIdentic());
                mapUse.put(2, World.world.getMap((short) 6826).getMapCopyIdentic());
                mapUse.put(3, World.world.getMap((short) 6828).getMapCopyIdentic());
                ...
                while (map.getMobGroups().size() > 0) {
                    if (!say)
                        SocketManager.GAME_SEND_cMK_PACKET_TO_MAP(map, "", -1, "Ganymede", "Pour entrer en combat, clique sur l'épouvantail.");
                    try {
                        Thread.sleep(1000);
                    } catch (InterruptedException ignored) {
                    }
