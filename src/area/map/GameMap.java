...
public class GameMap {
    ...
    public void deleteMobsOnMap() {
        ...
        for (Integer id : idsBorrar) {
            getMobGroups().remove(id);
        }
    ...
    public boolean isAggroByMob(Player perso, int cell) {
        ...
        for (Monster.MobGroup group : this.mobGroups.values()) {
            ...
            {
                if (World.world.getConditionManager().validConditions(perso, group.getCondition()))
                    return true;
            }
        }
        return false;
    ...
    public void panelTriggers(Player perso, Boolean mostrar) {
        StringBuilder str = new StringBuilder();
            ...
            if (c.getOnCellStop() != null) {
                for (Action a : c.getOnCellStop()) {
                    if (a.getId() == 0) {
                        ...
                        str.append("|").append(signe).append(c.getId()).append(";0;11");
                    }
                }
            }
        }
    ...
    public Fight newDeathmatch(final Player init1, final Player init2, final DeathMatch type) {
        if (init1.getFight() != null || init2.getFight() != null) {
            return null;
        }
        int id = 1;
    ...
    public Fight newKolizeum(final TeamMatch type) {
        int id = 1;
        if (!this.fights.isEmpty()) {
            id = (int) this.fights.toArray()[this.fights.size() - 1] + 1;
        }
        final Fight f = new Fight(type, id, this);
        this.fights.add(f);
        return f;
    }
    ...
    public void startFightVersusProtectors(Player player, Monster.MobGroup group) {
        ...
        if (!this.fights.isEmpty())
            id = ((Fight) (this.fights.toArray()[this.fights.size() - 1])).getId() + 1;
        this.fights.add(new Fight(id, this, player, group, Constant.FIGHT_TYPE_PVM));
        SocketManager.GAME_SEND_MAP_FIGHT_COUNT_TO_MAP(this);
    }
