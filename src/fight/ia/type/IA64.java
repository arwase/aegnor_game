 ...
 */
public class IA64 extends AbstractNeedSpell {

    public IA64(Fight fight, Fighter fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if (this.fighter.getCurPa(this.fight) > 0 && L == null && C == null) {
                int value = Function.getInstance().attackBondIfPossible(this.fight, this.fighter, ennemy);
                if (value != 0) {
                    time = value;
                    action = true;
                    L = Function.getInstance().getNearestEnnemynbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
            ...
            if (this.fighter.getCurPa(this.fight) > 0 && L != null && C == null && !action) {
                int value = Function.getInstance().attackIfPossible(fight, this.fighter, this.highests);
                if (value != 0) {
                    time = value;
                    action = true;
                }
