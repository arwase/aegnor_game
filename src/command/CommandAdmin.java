...
public class CommandAdmin extends AdminUser {
    ...
    public void command(String command, String[] infos, String msg) {
        if (command.equalsIgnoreCase("CHALL")) {
        ...
        } else if (command.equalsIgnoreCase("ADDFIGHTPOS") || command.equalsIgnoreCase("ADD_FIGHT_POS")) {
            int team = -1;
            int cell = -1;
            try {
            ...
            this.sendMessage("subArea : "
                    ...
                    + "\nsuperArea : "
                    + superArea);
        } else if (command.equalsIgnoreCase("PANEL_ADMIN")) {
            try {
            ...
            try {
                ...
                if (limite < 0) {
                    SocketManager.GAME_SEND_MESSAGE_CONSOLE(getPlayer(), "Arguments incorrectes");
                    return;
                }
                getPlayer().getCurMap().setMaxGroupNumb(limite);
                if (!Database.getDynamics().getMapData().updateMaxNumGroup(getPlayer().getCurMap(), limite)) {
                    SocketManager.GAME_SEND_MESSAGE_CONSOLE(getPlayer(), "Une erreur est survenu lors de l'actualisation des la BDD");
                    return;
                }
            ...
            try {
                ...
                getPlayer().getCurMap().setMaxGroupLimit(limite);
                if (!Database.getDynamics().getMapData().updateMaxSizeGroup(getPlayer().getCurMap(), limite)) {
                    SocketManager.GAME_SEND_MESSAGE_CONSOLE(getPlayer(), "Une erreur est survenu lors de l'actualisation des la BDD");
                    return;
                }
            ...
            try {
                ...
                try {
                    ...
                    SocketManager.GAME_SEND_MESSAGE_CONSOLE(getPlayer(), "Arguments invalides");
                    return;
                }
                ...
                GameMap mapa = getPlayer().getCurMap();
                try {
                    if (infos.length > 6) {
                        mapa = World.world.getMap(Short.parseShort(infos[6]));
                    }
                } catch (Exception ignored) {
                }

                mapa.addEndFightAction(tipo, new Action(accionID, args, "", mapa));
                strB.append("L'action de combat de fin a été ajoutée, mapID: ").append(mapa.getId()).append(", type: ").append(tipo).append(", actionID: ").append(accionID).append(", args: ").append(args).append(" condition: ").append(condicion);
                if (Database.getDynamics().getEndFightActionData().add(Integer.parseInt(String.valueOf(mapa.getId())), tipo, accionID, args, condicion)) {
                    strB.append(" à la BDD");
                }
            ...
            try {
                ...
                NpcTemplate npcMod = World.world.getNPCTemplate(id);
                if (npcMod == null) {
                    SocketManager.GAME_SEND_MESSAGE_CONSOLE(getPlayer(), "NPC $id no existe");
                }
                npcMod.ModifyNPC(sexo, escalaX, escalaY, gfxID, color1, color2, color3);
                npcMod.setAccesories(arma, sombrero, capa, mascota, escudo);
                Database.getDynamics().getNpcTemplateData().updateNPCSkin(npcMod);
            ...
            try {
                ...
                Monster mobModelo = World.world.getMonstre(id);
                if (mobModelo == null) {
            ...
            try {
                ...
                String condInicio = "";
                String grupoData = "";
                int segundosRespawn = 0;
                int tipoGrupoMob = 0;
                try {
                    grupoData = infos[1];
                    tipoGrupoMob = Integer.parseInt(infos[2]);
                } catch (Exception e) {
                    SocketManager.GAME_SEND_MESSAGE_CONSOLE(getPlayer(), "Arguments incorrects");
                    return;
                }
                ...
                try {
                    condInicio = infos[3].replace("menor", "<");
                    condInicio = condInicio.replace("mayor", ">");
                    condInicio = condInicio.replace("igual", "=");
                    condInicio = condInicio.replace("diferente", "!");
                } catch (Exception ignored) {
                }

                try {
                    condUnirse = infos[4].replace("menor", "<");
                    condUnirse = condUnirse.replace("mayor", ">");
                    condUnirse = condUnirse.replace("igual", "=");
                    condUnirse = condUnirse.replace("diferente", "!");
                } catch (Exception ignored) {
                }
                ...
                Monster.TipoGrupo tipoGrupo = Constant.getTipoGrupoMob(tipoGrupoMob);
                if (tipoGrupo == Monster.TipoGrupo.NORMAL) {
                    tipoGrupo = Monster.TipoGrupo.FIJO;
                }
                MobGroup grupoMob = getPlayer().getCurMap().addStaticGroups(getPlayer().getCurCell().getId(), grupoData, true);
            ...
            try {
                ...
                try {
                    ...
                    SocketManager.GAME_SEND_MESSAGE_CONSOLE(getPlayer(), "Erreur arguments");
                    return;
                }
            ...
            try {
                ...
                if (accionID <= -3) {
                    SocketManager.GAME_SEND_MESSAGE_CONSOLE(getPlayer(), "ActionID incorrects");
                    return;
                }
                GameCase celda = getPlayer().getCurCell();
                GameMap mapa = null;
                String[] mapcell = infos[3].split(",");
                try {
                    if (infos.length > 3) {
                        mapa = World.world.getMap(Short.parseShort(mapcell[0]));
                        celda = World.world.getMap(Short.parseShort(mapcell[0])).getCase(Integer.parseInt(mapcell[1]));
                    }
                } catch (Exception E) {
                    mapa = getPlayer().getCurMap();
                ...
                if (Database.getDynamics().getScriptedCellData().update((int) mapa.getId(), (int) celda.getId(), accionID, 1, args, condicion)) {
                    ...
                    celda.addOnCellStopAction(accionID, args, condicion, mapa);
                    SocketManager.send(getPlayer(), "La map: " + mapa.getId() + ", cell: " + celda.getId() + ", a acquérie l'action : " + accionID + ", args: " + args + ", et la condition (4to arg): " + condicion);
                } else {
                    SocketManager.send(getPlayer(), "Le trigger n'a pas pu recevoir l'action");
                }
            } catch (Exception e) {
