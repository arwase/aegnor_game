    ...
    public void buy(Player player)//Acheter une maison
    {
        ...
        if (house.getOwnerId() > 0) {
            ...
            if (seller.getCurrentPlayer() != null)//FIXME: change the packet (Im)
                SocketManager.GAME_SEND_MESSAGE(seller.getCurrentPlayer(), "Une maison vous appartenant à été vendue " + house.getSale() + " kamas.");
    ...
    public void parseHG(Player P, String packet) {
        ...
        if (packet != null) {
            if (packet.charAt(0) == '+') {
                ...
                Database.getDynamics().getHouseData().updateGuild(h, P.getGuild().getId(), 0);
                parseHG(P, null);
