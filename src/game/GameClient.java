...
public class GameClient {
    ...
    private void parseAccountPacket(String packet) {
        switch (packet.charAt(1)) {
            ...
            case 'R':
                ...
                break;
            case 'p': //reset stats and nerrf spell
                switch (packet.charAt(2)) {
                    case 'c':
                        player.Restat_Stats(false);
                        SocketManager.GAME_SEND_MESSAGE(player, "Vos caractéristiques ont été réinitialisées.", "2997F1");
                        break;
                    case 's':
                        ...
                        if (player.NerfSpell(id)) {
                            ...
                            SocketManager.GAME_SEND_MESSAGE(player, "Votre sort n'a pas été abaissé en niveau une erreur est survenue.", "2997F1");
                            break;
                        }
                }
                break;
    ...
    private void getGifts(String packet) {
        ...
        if (!gifts.isEmpty()) {
            ...
            for (String object : gifts.split(";")) {
                ...
                if (data.isEmpty())
                    data = "1~" + Integer.toString(id, 16) + "~" + Integer.toString(qua, 16) + "~~" + World.world.getObjTemplate(id).getStrTemplate();
                else
                    data += ";1~" + Integer.toString(id, 16) + "~" + Integer.toString(qua, 16) + "~~" + World.world.getObjTemplate(id).getStrTemplate();
                if (item == -1) item = id;
            }
    ...
    private void authorisedCommand(String packet) throws InterruptedException {
        ...
        Logging.command.info(this.getAccount().getCurrentIp() + " : " + this.getAccount().getName() + " > " + this.getPlayer().getName() + " > " + packet.substring(2));
    ...
    private void tchat(String packet) {
        ...
        switch (packet.charAt(2)) {
            ...
            case '*'://Defaut
                ...
                Logging.tchat.info("Default" + this.player.getName() + " > Map " + this.player.getCurMap().getId() + " > " + msg);
            ...
            case '#'://Canal Equipe
                ...
                if (this.player.getFight() != null) {
                    ...
                    if (team == -1)
                        return;
                    Logging.tchat.info("Team" + this.player.getName()
            ...
            case '$'://Canal groupe
                ...
                if (this.player.getParty() == null)
                    break;
                msg = packet.split("\\|", 2)[1];
                Logging.tchat.info(("Party" + this.player.getName()
            ...
            case ':'://Canal commerce
                ...
                if (this.player.cantCanal()) {
                    ...
                    msg = packet.split("\\|", 2)[1];
                    Logging.tchat.info(("Trade" + this.player.getName()
                            + " > " + msg));
                    if (this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF) != null)
                        if (this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId() == 10844)
                            msg = Formulas.translateMsg(msg);
            ...
            case '@'://Canal Admin
                if (this.player.getGroupe() == null)
                    return;
                msg = packet.split("\\|", 2)[1];
                Logging.tchat.info(("Admin " + this.player.getName()
                        + " > " + msg));
            ...
            case '?'://Canal recrutement
                ...
                if (this.player.cantCanal()) {
                    ...
                    msg = packet.split("\\|", 2)[1];
                    Logging.tchat.info(("Recruitment" + this.player.getName()
                            + " > " + msg));
                    if (this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF) != null)
                        if (this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId() == 10844)
                            msg = Formulas.translateMsg(msg);
            ...
            case '%'://Canal guilde
                ...
                if (this.player.getGuild() == null)
                    return;
                msg = packet.split("\\|", 2)[1];
                Logging.tchat.info(("Guild" + this.player.getName()
            ...
            case '!'://Alignement
                ...
                msg = packet.split("\\|", 2)[1];
                Logging.tchat.info(("Align" + this.player.getName()
                        + " > " + msg));
                if (this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF) != null)
                    if (this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId() == 10844)
                        msg = Formulas.translateMsg(msg);
            ...
            default:
                ...
                if (!(nom.length() <= 1)) {
                    ...
                    Logging.tchat.info(("Private" + this.player.getName() + " à " + target.getName() + " > " + msg));
                    if (this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF) != null)
                        if (this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId() == 10844)
                            msg = Formulas.translateMsg(msg);
    ...
    private void parseDialogPacket(String packet) {
        ...
        if (party != null && this.player.getFight() == null && party.getMaster() != null && party.getMaster().getName().equals(this.player.getName())) {
            TimerWaiter.addNext(() -> party.getPlayers().stream().filter((follower1) -> party.isWithTheMaster(follower1, false))
                    .forEach(follower -> follower.getGameClient().parseDialogPacket(packet)), 0, TimeUnit.SECONDS, TimerWaiter.DataType.CLIENT);
        }
    }
    ...
    private void buy(String packet) {
        ...
        ExchangeAction<?> checkExchangeAction = this.player.getExchangeAction();
        if (checkExchangeAction == null || !(checkExchangeAction.getValue() instanceof Integer) || (checkExchangeAction.getType() != ExchangeAction.TRADING_WITH_OFFLINE_PLAYER && checkExchangeAction.getType() != ExchangeAction.TRADING_WITH_NPC))
            return;
    ...
    private synchronized void movementItemOrKamas(String packet) {
        ...
        switch (this.player.getExchangeAction().getType()) {
            ...
            case ExchangeAction.TRADING_WITH_COLLECTOR:
                ...
                Database.getStatics().getGuildData().update(this.player.getGuild());
                break;
    ...
    private void recursiveBreakingObject(BreakingObject breakingObject, final int i, int count) {
        ...
        }, 1000, TimeUnit.MILLISECONDS, TimerWaiter.DataType.CLIENT);
    }
    ...
    private void offlineExchange() {
        ...
        if (taxe < 0) {
            SocketManager.GAME_SEND_MESSAGE(this.player, "Erreur de mode marchand, la somme est négatif.");
            return;
        }
            ...
            case ExchangeAction.TRADING_WITH_COLLECTOR:
                ...
                for (Player loc : World.world.getGuild(collector.getGuildId()).getPlayers()) {
                    if (loc != null && loc.isOnline()) {
                        SocketManager.GAME_SEND_gITM_PACKET(loc, entity.Collector.parseToGuild(loc.getGuild().getId()));
                        String str = "";
    ...
    private void parseGamePacket(String packet) {
        switch (packet.charAt(1)) {
            ...
            case 'd':
                ...
                break;
            case 'D':
                SocketManager.GAME_SEND_MAPDATA_COMPLETE(this.player);
                break;
            case 'f':
                setFlag(packet);
                break;
            ...
            case 1://Deplacement
                ...
                if (party != null && this.player.getFight() == null && party.getMaster() != null && party.getMaster().getName().equals(this.player.getName())) {
                    TimerWaiter.addNext(() -> party.getPlayers().stream()
                            ...
                            }), 0, TimeUnit.MILLISECONDS, TimerWaiter.DataType.CLIENT);
                }
                break;
        ...
        if (this.player.getFight() == null) {
            ...
            if (result == 0) {
                if (targetCell.getObject() != null) {
                    logger.debug("Object Interactif {} sur la cell {} en fin de deplacement",
                            targetCell.getObject().getId(), targetCell.getId());
            ...
            if (this.player.walkFast) {
                ...
                if (targetCell.getObject() != null) {
                    logger.debug("Object Interactif {} sur la cell {} apres action",
                            targetCell.getObject().getId(), targetCell.getId());
    ...
    private void placeTaxCollector() {
        ...
        Collector collector = new Collector(id, map.getId(), this.player.getCurCell().getId(), (byte) 3, guild.getId(), n1, n2, this.player, System.currentTimeMillis(), "", 0, 0);
        World.world.addCollector(collector);
        SocketManager.GAME_SEND_ADD_PERCO_TO_MAP(map);
        SocketManager.GAME_SEND_STATS_PACKET(this.player);
        Database.getDynamics().getCollectorData().add(id, map.getId(), guild.getId(), this.player.getId(), System.currentTimeMillis(), this.player.getCurCell().getId(), 3, n1, n2);

        for (Player player : guild.getPlayers()) {
            if (player != null && player.isOnline()) {
                SocketManager.GAME_SEND_gITM_PACKET(player, entity.Collector.parseToGuild(player.getGuild().getId()));
                String str = "";
    ...
    private void banToGuild(String name) {
        ...
        //si quitter
        {
            ...
            {
                SocketManager.GAME_SEND_MESSAGE(this.player, "Vous devez mettre un autre meneur pour devoir quitter la guilde !");
                return;
            }
    ...
    private void dropObject(String packet) {
        ...
        Logging.objects.debug("Dropping : " + this.player.getName() + " a jeté [" + obj.getTemplate().getId() + "@" + obj.getGuid() + ";" + qua + "]");
        SocketManager.GAME_SEND_Ow_PACKET(this.player);
    ...
    private synchronized void movementObject(String packet) {
        ...
        try {
            ...
            if (this.player.deathMatch != null && this.player.getEquippedObjects().size() == 1) {
                boolean bool = false;
                for (GameObject clone : this.player.getEquippedObjects())
                    if (object.getGuid() == clone.getGuid())
                        bool = true;
                if (bool) {
            ...
            if (position == Constant.ITEM_POS_FAMILIER && object.getTemplate().getType() != Constant.ITEM_TYPE_FAMILIER && this.player.getObjetByPos(position) != null) {
                ...
                if (!object.getTemplate().getConditions().equalsIgnoreCase("") && !World.world.getConditionManager().validConditions(this.player, object.getTemplate().getConditions())) {
    ...
    public void addAction(GameAction GA) {
        ...
        logger.debug("Game > Create action id : " + GA.id);
        logger.debug("Game > Packet : " + GA.packet);
    }

    public synchronized void removeAction(GameAction GA) {
        if (GA.actionId == 1)
            walk = false;
        logger.info("Game >  Delete action id : " + GA.id);
